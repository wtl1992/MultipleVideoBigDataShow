$(function () {
    ...
    for (var i=0;i<100;i++){
        if (i == pageIndex - 1){
            sPageHtml += "<li class='current'><a href=\"m/movieResult?pageIndex="+(i+1)+"\">"+(i+1)+"</a></li>";
        }
        else{
            sPageHtml += "<li><a href=\"m/movieResult?pageIndex="+(i+1)+"\">"+(i+1)+"</a></li>";
        }
    ...
    $("div.searchArea p input").on("keydown",function (event) {
        $("div.searchArea a").attr("href","m/movieSearchResult?keyword="+$(this).val());
        if (event.keyCode === 13 && $(this).val() !== ""){
            window.location = "m/movieSearchResult?keyword="+$(this).val();
        }
    });
