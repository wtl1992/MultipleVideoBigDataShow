$(function () {
    ...
    for (var i=0;i<100;i++){
        if (i == pageIndex - 1){
            sPageHtml += "<li class='current'><a href=\"m/imageResult?keyword=" + keyword+"&pageIndex="+(i+1)+"&pageSize=60\">"+(i+1)+"</a></li>";
        }
        else{
            sPageHtml += "<li><a href=\"m/imageResult?keyword=" + keyword+"&pageIndex="+(i+1)+"&pageSize=60\">"+(i+1)+"</a></li>";
        }
    ...
    $("div.searchArea p input").on("keydown",function (event) {
        $("div.searchArea a").attr("href","m/imageResult?keyword=" + $(this).val()+"&pageIndex=1&pageSize=60");
        if (event.keyCode === 13 && $(this).val() !== ""){
            window.location = "m/imageResult?keyword=" + $(this).val()+"&pageIndex=1&pageSize=60";
        }
    });
