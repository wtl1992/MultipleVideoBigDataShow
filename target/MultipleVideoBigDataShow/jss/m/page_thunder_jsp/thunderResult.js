$(function () {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            for (var i=0;i<data[0]["pageCount"];i++){
                if (i == pageIndex - 1){
                    sPageHtml += "<li class='current'><a href=\"m/getThunderPagingResult?classify="+classify+"&pageIndex="+(i+1)+"\">"+(i+1)+"</a></li>";
                }
                else{
                    sPageHtml += "<li><a href=\"m/getThunderPagingResult?classify="+classify+"&pageIndex="+(i+1)+"\">"+(i+1)+"</a></li>";
                }
            ...
            for (var i=0;i<thunders.length;i++){
                sthunderHtml += "<li>\n" +
                    "                <p class=\"title\">\n" +
                    "                    <a href=\""+("m/getThunderDownloadResult?url="+window.encodeURIComponent(thunders[i]["downloadPageUrl"]))+"\">"+thunders[i]["title"]+"</a>\n" +
                    "                </p>\n" +
    ...
    for (var i=0;i<classifys.length;i++){
        if (classify == classifys[i]){
            sclassifyHtml += "<li class='current'>\n" +
                "                <a href=\"m/getThunderPagingResult?classify="+classifys[i]+"&pageIndex=1\">"+classifys[i]+"</a>\n" +
                "            </li>";
        }
        else{
            sclassifyHtml += "<li>\n" +
    ...
    $("div.searchArea p input").on("keydown",function (event) {
        $("div.searchArea a").attr("href","m/getThunderResult?keyword="+$(this).val()+"&typeid="+$("select").val()+"&pageIndex=1");
        if (event.keyCode === 13 && $(this).val() !== ""){
            window.location = "m/getThunderResult?keyword="+$(this).val()+"&typeid="+$("select").val()+"&pageIndex=1";
        }
    });
