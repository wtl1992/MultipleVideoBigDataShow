$(function () {
    ...
    for (var i=0;i<20;i++){
        if (i == pageIndex - 1){
            sPageHtml += "<li class='current'><a href=\"musicResult?keyword="+keyword+"&pageIndex="+(i+1)+"&pageSize=20\">"+(i+1)+"</a></li>";
        }
        else{
            sPageHtml += "<li><a href=\"musicResult?keyword="+keyword+"&pageIndex="+(i+1)+"&pageSize=20\">"+(i+1)+"</a></li>";
        }
    ...
    $("div.searchArea p input").on("keydown",function (event) {
        ...
        if (event.keyCode === 13 && $(this).val() !== ""){
            window.location = "musicResult?keyword=" + $(this).val()+"&pageIndex=1&pageSize=20";
        }
    });
