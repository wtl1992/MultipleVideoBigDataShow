$(function () {
    ...
    for (var i=0;i<100;i++){
        if (i == pageContext - 1){
            sPageHtml += "<li class='current'><a href=\"m/appResult?categoryId=" + categoryId+"&pageSize="+pageSize+"&pageContext="+(i+1)+"\">"+(i+1)+"</a></li>";
        }
        else{
            sPageHtml += "<li><a href=\"m/appResult?categoryId=" + categoryId+"&pageSize="+pageSize+"&pageContext="+(i+1)+"\">"+(i+1)+"</a></li>";
        }
    ...
    for (var i=0;i<keywords.length;i++){
        if (categoryId == categoryIds[i]){
            sclassifyHtml += "<li class='current'>\n" +
                "                <a href=\"m/appResult?categoryId=" + categoryIds[i]+"&pageSize="+pageSize+"&pageContext=1\">"+keywords[i]+"</a>\n" +
                "            </li>";
        }
        else{
            sclassifyHtml += "<li>\n" +
    ...
    $("div.searchArea p input").on("keydown",function (event) {
        $("div.searchArea a").attr("href","m/appSearchResult?keyword="+$(this).val());
        if (event.keyCode === 13 && $(this).val() !== ""){
            window.location = "m/appSearchResult?keyword="+$(this).val();
        }
    });
